    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-xl h-fit max-w-5xl mx-auto select-none animate-fade-in relative">
      ...
      {step === 1 && (
        <div className="space-y-6">
          ...
          {/* 3D specs card block */}
          {activeVehicle && specs && (
            <div className="bg-slate-50 dark:bg-slate-950/20 p-5 rounded-3xl border border-slate-100 dark:border-slate-850 select-none animate-fade-in">
              <div className="flex items-center gap-2 mb-4">
                <Car className="w-5 h-5 text-indigo-500" />
                <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider">Automobile Premium Specs</h4>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
                {/* 3D Showcase Column */}
                <div 
                  className="lg:col-span-1 relative bg-gradient-to-br from-indigo-900 via-slate-950 to-indigo-950 rounded-2xl p-5 h-[275px] overflow-hidden shadow-2xl border border-indigo-500/25 cursor-pointer flex flex-col justify-between group transition-all duration-300 ease-out"
                  style={{
                    perspective: 1000,
                    transform: `rotateX(${cardTilt.x}deg) rotateY(${cardTilt.y}deg) scale(1.02)`,
                ...
                >
                  ...
                  <div className="flex justify-between items-start z-10" style={{ transform: 'translateZ(30px)' }}>
                    <div>
                      <h5 className="text-white font-extrabold text-sm uppercase tracking-wide">{activeVehicle.model}</h5>
                      <span className="text-[10px] text-indigo-250 font-bold uppercase tracking-widest">{activeVehicle.make}</span>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md px-2.5 py-1 rounded-lg text-white font-mono text-[9px] uppercase font-bold tracking-wider border border-white/20">
                      {activeVehicle.vehicleNumber}
                    </div>
                  </div>

                  {/* Vector Vehicle Silhouette / View switcher */}
                  <div 
                    className={`w-full flex justify-center items-center my-1.5 transform transition-all duration-300 z-10 ${
                      activeView === '360° View' ? 'animate-float' : 'group-hover:scale-105'
                    }`} 
                    style={{ transform: 'translateZ(50px)' }}
                  >
                    {activeView === 'Exterior' && (
                      <div className="w-full">
                        {specs.bodyType === 'suv' && <CarSilhouetteSUV />}
                        {specs.bodyType === 'sedan' && <CarSilhouetteSedan />}
                        {specs.bodyType === 'hatchback' && <CarSilhouetteHatchback />}
                        {specs.bodyType === 'ev' && <CarSilhouetteEV />}
                      </div>
                    )}
                    {activeView === 'Interior' && (
                    ...
                    {activeView === '360° View' && (
                      <div className="w-full relative overflow-hidden">
                        {/* Slow Horizontal Spin/Float effect */}
                        ...
                        <div className="absolute inset-x-0 h-0.5 bg-cyan-400/80 blur-[1px] shadow-[0_0_8px_rgba(34,211,238,0.8)] animate-bounce top-1/2" />
                      </div>
                    )}
                  </div>

                  {/* Circular 3D Neon Podium Base */}
                  <div className="relative w-full h-8 mt-[-15px] flex justify-center items-center z-0">
                    {/* Rotating outer scan line if in 360 view */}
                    <div className={`absolute w-[80%] h-4 bg-transparent border-2 border-indigo-400/40 rounded-full blur-[0.5px] shadow-[0_0_12px_rgba(99,102,241,0.6)] transform rotateX(60deg) ${
                      activeView === '360° View' ? 'animate-spin border-dashed border-cyan-400' : ''
                    }`} />
                    <div className="absolute w-[70%] h-3 bg-indigo-500/20 rounded-full blur-sm transform rotateX(60deg)" />
                  </div>

                  {/* Brand logo & MVSS details */}
                  <div className="flex justify-between items-center z-10" style={{ transform: 'translateZ(30px)' }}>
                    <div className="flex items-center gap-1.5 bg-white/5 backdrop-blur-md px-2 py-1 rounded-lg border border-white/10 text-white/70 text-[9px] font-bold uppercase tracking-wider">
                      {BRAND_LOGOS[activeVehicle.make.toLowerCase()] ? (
                        BRAND_LOGOS[activeVehicle.make.toLowerCase()]('w-3.5 h-3.5')
                      ) : (
                  ...
                  {/* Dynamic View Control Tabs */}
                  <div className="flex justify-center gap-1 mt-2.5 border-t border-white/10 pt-2.5 z-10" style={{ transform: 'translateZ(20px)' }}>
                    {[
                      ...
                      <button
                        ...
                        className={`px-2 py-0.5 rounded text-[8px] font-extrabold uppercase tracking-wider transition-all duration-300 ${
                          activeView === tab.id
                            ? 'bg-indigo-650 text-white shadow-md shadow-indigo-600/30 border border-indigo-500/30'
                            : 'bg-white/5 text-white/45 hover:bg-white/10 border border-white/5 hover:text-white'
                        }`}
                      >
                        {tab.label}
                      </button>
                ...
                {/* Specification Dashboard Column */}
                <div className="lg:col-span-2 grid grid-cols-2 gap-4">
                  {/* Spec 1: Engine */}
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3.5 rounded-2xl flex items-center gap-3 shadow-sm hover:border-indigo-500/20 transition-all duration-300">
                    <div className="w-9 h-9 rounded-xl bg-indigo-50 dark:bg-indigo-950/40 flex items-center justify-center text-indigo-650 dark:text-indigo-400 shrink-0">
                      <Gauge className="w-5 h-5" />
                    </div>
                    <div className="overflow-hidden">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Engine Spec</span>
                      <span className="text-xs font-extrabold text-slate-850 dark:text-white truncate block uppercase" title={specs.engine}>
                        {specs.engine}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3.5 rounded-2xl flex items-center gap-3 shadow-sm hover:border-indigo-500/20 transition-all duration-300">
                    ...
                    <div className="overflow-hidden">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Transmission</span>
                      <span className="text-xs font-extrabold text-slate-855 dark:text-white truncate block uppercase">
                        {specs.transmission}
                      </span>
                    </div>
                  </div>
                  ...
                  {/* Spec 3: Fuel Type */}
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3.5 rounded-2xl flex items-center gap-3 shadow-sm hover:border-indigo-500/20 transition-all duration-350">
                    <div className="w-9 h-9 rounded-xl bg-indigo-50 dark:bg-indigo-950/40 flex items-center justify-center text-indigo-650 dark:text-indigo-400 shrink-0">
                      <Flame className="w-5 h-5" />
                    </div>
                    <div className="overflow-hidden">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Fuel System</span>
                      <span className="text-xs font-extrabold text-slate-855 dark:text-white truncate block uppercase">
                        {specs.fuelType}
                      </span>
                    </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3.5 rounded-2xl flex items-center gap-3 shadow-sm hover:border-indigo-500/20 transition-all duration-350">
                    ...
                    <div className="overflow-hidden">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Model Year</span>
                      <span className="text-xs font-extrabold text-slate-855 dark:text-white truncate block uppercase">
                        {specs.year}
                      </span>
                    </div>
